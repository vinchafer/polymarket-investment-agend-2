...
class TelegramNotifier:
    ...
    def send_message(self, text: str, parse_mode: str = "HTML") -> bool:
        ...
        # === A2 COPY-ARM: bet-only alert filter (env-gated, dormant by default) ===
        # A2 soll auf Telegram NUR Bet-Placements senden. Alle direkten
        # send_message-Aufrufer (health, 6h-summary, resolution, learning, router)
        # werden hier zentral gefiltert. Ohne A2_ALERTS_BET_ONLY = identisch zu A1.
        import os
        if os.getenv("A2_ALERTS_BET_ONLY", "").strip().lower() in ("1", "true", "yes"):
            if "BET PLACED" not in (text or ""):
                logger.debug("A2 bet-only filter: suppressed non-bet alert")
                return False
