#!/usr/bin/env python3
"""Agent 2 (copy-arm) — Read-Only REST API. Port: 8767"""
...
DB_PATH = "/root/polymarket-agent2/trading_log.db"
STARTING_CAPITAL = 50.0
PORT = 8768

app = FastAPI(title="Agent 2 (copy-arm) Read-Only API", version="1.0.0")
    ...
    try:
        ...
        c.execute("""
            SELECT id, market_condition_id, market_question, action,
                   ...
                   confidence_at_bet, 0 AS days_to_resolution_at_bet
            FROM trades
            WHERE resolved = 0
