...
def _groq_challenge(payload: dict) -> dict:
    ...
    parsed, provider = _get_devil_router().decide_json(prompt)
    if not parsed:
        logger.warning("Devil: all LLM providers down, applying safe defaults")
        return {
            "fatal_flaw_found": False,
            "flaws": ["all LLM providers down"],
            "override_to_skip": False,
            "confidence_adjustment": 0.0,
            "final_verdict": "PROCEED",
            "override_reason": "",
        }
    parsed.setdefault("fatal_flaw_found", False)
    parsed.setdefault("flaws", [])
    parsed.setdefault("override_to_skip", False)
    parsed.setdefault("confidence_adjustment", 0.0)
    parsed.setdefault("final_verdict", "PROCEED")
    parsed.setdefault("override_reason", "")
    return parsed
