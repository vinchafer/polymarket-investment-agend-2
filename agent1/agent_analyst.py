...
def _decide(payload: dict, research: str) -> tuple:
    """Returns (decision_dict, provider_used)."""
    router = _get_router()
    prompt = _build_prompt(payload, research)
    parsed, provider = router.decide_json(prompt)
    if not parsed:
        return {
            ...
            "confidence": "low",
            "action": "SKIP",
            "reasoning": "All LLM providers down",
            "key_risk": "LLM unavailable",
        }, "none"
    parsed.setdefault("contradicting_info", False)
    parsed.setdefault("has_value", True)
    parsed.setdefault("confidence", "medium")
    parsed.setdefault("action", "SKIP")
    parsed.setdefault("reasoning", "")
    parsed.setdefault("key_risk", "")
...
class AnalystAgent:
    ...
    def analyze_one(self, event: dict):
        ...
        # Parallel research — keyfree (DuckDuckGo + GDELT + Reddit) + ESPN/NewsAPI
        sources_used = []
        parts = []
        with ThreadPoolExecutor(max_workers=5) as ex:
            futures = {
                ex.submit(_espn_fetch, market_question): "ESPN",
                ex.submit(_duckduckgo_fetch, market_question): "DuckDuckGo",
                ex.submit(_gdelt_fetch, market_question): "GDELT",
                ex.submit(_reddit_fetch, market_question): "Reddit",
                ex.submit(_newsapi_fetch, market_question): "NewsAPI",
            }
            for fut in as_completed(futures, timeout=15):
                ...
                try:
                    result = fut.result()
                    if result:
                        parts.append(f"[{src}]: {result}")
                        sources_used.append(src.lower())
                except Exception as e:
                    logger.debug(f"Analyst: {src} future error: {e}")
